...
export default function SubmissionsDashboard({ initialSubmissions, teacherId, refreshTrigger }: SubmissionsDashboardProps) {
  ...
  // Dynamic Polling for Pending or Processing submissions
  useEffect(() => {
    ...
    console.log(`Setting up polling for ${activeSubmissions.length} active submissions...`);
      ...
      const updatedSubmissions = await Promise.all(
        submissions.map(async (sub) => {
          if (sub.status === "PENDING" || sub.status === "PROCESSING") {
            const statusRes = await getSubmissionStatus(sub.id);
            if (statusRes.success && statusRes.data) {
              const freshSub = statusRes.data as unknown as Submission;
              if (freshSub.status !== sub.status) {
                updatedAny = true;
                return freshSub;
              }
            }
